...
func (c *CourseClassController) Post() {
	...
	if valid.HasErrors() {
		...
		if _, err := models.AddCourseClass(&course, content); err == nil {
...
// @Description 一体机课程表
// @Param  class_id  query  int  true  "班级id"
// @Param  time  query  string  true  "日期（年-0月-0日）"
// @Success 0       {json}   JSONStruct
// @Failure 1005 获取失败
// @router /class_daycourse [get]
func (c *CourseClassController) GetClassCourseware() {
	class_id, _ := c.GetInt("class_id")
	time := c.GetString("time")
	if list, err := models.GetClassCourseware(class_id, time); err == nil {
		c.Data["json"] = JSONStruct{"success", 0, list, "获取成功"}
	} else {
		c.Data["json"] = JSONStruct{"error", 1005, nil, "获取失败"}
	}
