...
func (c *StudentController) GetStudentClass() {
	prepage, _ := c.GetInt("per_page", 1000)
...
func (c *StudentController) Post() {
	...
	IdNumber := c.GetString("id_number")
	kindergarten_id, _ := c.GetInt("kindergarten_id")
	emergency_contact := c.GetString("emergency_contact")
	emergency_contact_phone := c.GetString("emergency_contact_phone")
	err := models.AddAppStudent(sex, age, name, phone, address, relation, emergency_contact, emergency_contact_phone, baby_id, kindergarten_id, ty, IdNumber, notice_id)
	if err != nil {
...
func (c *StudentController) UpdateStudent() {
	...
	if valid.HasErrors() {
		...
		err := models.SaveStudent(id, student, kinship, ty, member_ids, is_principal, organizational_id, class)
		if err != nil {
			c.Data["json"] = JSONStruct{"error", 1003, nil, err.Error()}
		} else {
...
func (c *StudentController) Invite() {
	student := c.GetString("student")
	kindergarten_id, _ := c.GetInt("kindergarten_id")
	err := models.Invite(student, kindergarten_id)
	if err != nil {
		c.Data["json"] = JSONStruct{"error", 1003, nil, err.Error()}
	} else {
...
// BabyKinderList ...
// @Title app baby状态列表-前台
// @Description app baby状态列表-前台
// @Param	page                  query	int	     false		"页数"
// @Param	per_page              query	int	     false		"每页显示条数"
// @Success 200 {object} models.Student
// @Failure 403
// @router /baby_list [get]
func (c *StudentController) BabyKinderList() {
	page, _ := c.GetInt("page")
	prepage, _ := c.GetInt("per_page", 20)
	kindergarten_id, _ := c.GetInt("kindergarten_id")
	v, err := models.BabyKinderList(page, prepage, kindergarten_id)
	if err != nil {
		c.Data["json"] = JSONStruct{"error", 1005, nil, "获取失败"}
	} else {
		c.Data["json"] = JSONStruct{"success", 0, v, "获取成功"}
...
// @Failure 403 :学生编号为空
// @router /:id [get]
func (c *StudentController) GetStudentOne() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v, err := models.GetStudentOne(id)
	if err != nil {
