...
func (c *KindergartenLifeController) Store() {
	...
	if valid.HasErrors() {
		...
		err := models.AddKindergartenLife(content, template, kindergarten_id, picture)
		if err != nil {
...
// @Failure 403
// @router /activity [get]
func (c *KindergartenLifeController) ActivityPicture() {
	kindergarten_id, _ := c.GetInt("kindergarten_id")
	valid := validation.Validation{}
	valid.Required(kindergarten_id, "kindergarten_id").Message("幼儿园编号不能为空")
	if valid.HasErrors() {
		...
		v, err := models.ActivityPicture(kindergarten_id)
		if err != nil {
			c.Data["json"] = JSONStruct{"error", 1005, err.Error(), "获取失败"}
		} else {
			c.Data["json"] = JSONStruct{"success", 0, v, "获取成功"}
