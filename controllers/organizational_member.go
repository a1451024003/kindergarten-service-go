...
func (c *OrganizationalMemberController) MyKindergarten() {
	...
	if valid.HasErrors() {
		...
		v, err := models.TeacherClass(kindergarten_id, class_type)
		if err != nil {
			c.Data["json"] = JSONStruct{"error", 1006, nil, err.Error()}
		} else {
			c.Data["json"] = JSONStruct{"success", 0, v, "获取成功"}
...
func (c *OrganizationalMemberController) OrganizationalTeachers() {
	organizational_id, _ := c.GetInt("organizational_id")
	valid := validation.Validation{}
	valid.Required(organizational_id, "organizational_id").Message("组织架构id不能为空")
	if valid.HasErrors() {
		...
		v, err := models.OrganizationalTeachers(organizational_id)
		if err != nil {
			c.Data["json"] = JSONStruct{"error", 1006, nil, err.Error()}
		} else {
			c.Data["json"] = JSONStruct{"success", 0, v, "获取成功"}
