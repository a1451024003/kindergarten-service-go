...
func (c *CourseInfoController) Add_info() {
	...
	if valid.HasErrors() {
		...
		if info, err := models.AddCourseInfo(&course); err == nil {
			c.Data["json"] = JSONStruct{"success", 0, info, "保存成功"}
		} else {
...
// @param 		begin_date			query  	string 	true		"开始时间"
// @param 		end_date			query  	string 	true		"结束时间"
// @router /editinfo [post]
func (c *CourseInfoController) Edit_info() {
	id, _ := c.GetInt("id")
	course_id, _ := c.GetInt("course_id")
	types, _ := c.GetInt("type")
	tearcher_id, _ := c.GetInt("tearcher_id")
	...
	if valid.HasErrors() {
		...
		if err := models.GetCourseInfoEdit(id, course); err == nil {
