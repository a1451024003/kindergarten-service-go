...
func (c *InspectController) Counts() {
	...
	v := healthy.Counts(kindergarten_id, class_id, role)
...
// @Failure 1001 补全信息
// @Failure 1003 获取失败
// @router /situationUrl [get]
func (c *InspectController) SituationUrl() {
	var f *healthy.Inspect
	bady_id, _ := c.GetInt("baby_id")
	...
	if works, err := f.SituationUrl(bady_id); err == nil {
		if works == nil {
			c.Data["json"] = JSONStruct{"success", 0, works, "此宝宝无异常记录"}
		} else {
			c.Data["json"] = JSONStruct{"success", 0, works, "获取成功"}
		}
	} else {
...
// @Failure 1001 		参数不能为空
// @Failure 1005 		获取失败
// @router /personalList/ [get]
func (c *InspectController) PersonalList() {
	var f *healthy.Inspect
	babys := c.GetString("babys")

	if personal, err := f.PersonalList(babys); err == nil {
		c.Data["json"] = JSONStruct{"success", 0, personal, "获取成功"}
	} else {
		c.Data["json"] = JSONStruct{"error", 1005, err, "获取失败"}
	}
