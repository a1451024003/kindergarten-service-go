...
func (c *WorkTaskController) Post() {
	...
	deadline, err := time.ParseInLocation("2006-01-02 15:04:05", deadlineS, local)
	if err != nil {
	...
	if res, err := wt.GetInfoById(); err == nil {
		for _, v := range res["operator"].([]task.WorkTasksOperator) {
			if v.Operator == uId {
				...
				res["courseware_id"] = v.CoursewareId
				res["courseware_thumb"] = v.CoursewareThumb

				break
			} else {
				res["operate_status"] = -1
				res["courseware_id"] = ""
				res["courseware_thumb"] = ""
			}
		}
		c.Data["json"] = JSONStruct{"success", 0, res, "获取成功"}
...
// @Param   courseware_name     	formData    string  false       "课件名称（当课件ID不为空的时候必传）"
// @Param   courseware_thumb     	formData    string  false       "课件缩略图（当课件ID不为空的时候必传）"
// @Success 0 {int} models.Feedback.Id
// @Failure 1001 参数验证
// @Failure 1003 操作失败
// @router /complete [put]
func (c *WorkTaskController) Complete() {
	...
	coursewareThumb := c.GetString("courseware_thumb")

	valid := validation.Validation{}
	valid.Required(taskId, "task_id").Message("任务ID不能为空")
	valid.Required(operator, "operator").Message("操作人不能为空")
	if coursewareId != "" {
		valid.Required(coursewareName, "courseware_name").Message("课件名称不能为空")
		valid.Required(coursewareThumb, "courseware_thumb").Message("课件缩略图不能为空")
	}
	if valid.HasErrors() {
		c.Data["json"] = JSONStruct{"error", 1001, "", valid.Errors[0].Message}
	...
	if err := wt.Complete(operator, coursewareId, coursewareName, coursewareThumb); err == nil {
		c.Data["json"] = JSONStruct{"success", 0, "", "操作成功"}
	} else if err == orm.ErrNoRows {
		c.Data["json"] = JSONStruct{"error", 1002, "", "任务不存在"}
	} else if err.Error() == "completed" {
		c.Data["json"] = JSONStruct{"error", 1006, "", "重复操作"}
	} else if err.Error() == "exceed" {
		c.Data["json"] = JSONStruct{"error", 1006, "", "禁止操作"}
	} else {
