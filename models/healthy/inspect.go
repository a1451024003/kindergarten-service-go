...
//统计
func Counts(kindergarten_id, class_id, role int) map[string]interface{} {
	o := orm.NewOrm()
	...
	if role != 1 && role != 0 {
		where += " AND class_id = " + strconv.Itoa(class_id)
	}
	type Counts struct {
		Num int
	}
	...
	if role != 1 && role != 0 {
		where1 += " AND class_id = " + strconv.Itoa(class_id)
	}
	where1 += " AND left(date,10) = '" + day_time + "'"
	where1 += " AND student_id != 0 "
	...
	if role != 1 && role != 0 {
		where2 += " AND class_id = " + strconv.Itoa(class_id)
	}
	...
	where2 += " AND healthy_inspect.student_id != 0 "
	...
	if role != 1 && role != 0 {
		where3 += " AND class_id = " + strconv.Itoa(class_id)
	}
	...
	where3 += " AND healthy_inspect.student_id != 0 "
	...
	if role != 1 && role != 0 {
		where4 += " AND class_id = " + strconv.Itoa(class_id)
	}
	where4 += " AND student_id != 0 "
...
func AddlistInspect(data string) (some_err []interface{}) {
	...
	if err := o.Read(&b); err == nil {
		var num Num
		sql := "select count(a.id) as num from healthy_inspect a where a.body_id = " + strconv.Itoa(i[0].BodyId) + " and a.student_id != 0"
	...
	if err := o.QueryTable("healthy_class").Filter("class_id", i[0].ClassId).Filter("body_id", i[0].BodyId).One(&c); err == nil {
		var num Num
		sql := "select count(a.id) as num from healthy_inspect a where a.body_id = " + strconv.Itoa(i[0].BodyId) + " and class_id=" + strconv.Itoa(i[0].ClassId) + " and a.student_id != 0"
		o.Raw(sql).QueryRow(&num)
...
//健康异常档案
func (f *Inspect) Abnormals(pType, types, role, page, perPage, kindergarten_id, class_id int, date, search string) (Page, error) {
	o := orm.NewOrm()
	var con []interface{}
	where := "healthy_inspect.student_id != 0 "
	where += "AND (healthy_inspect.abnormal != '' Or healthy_inspect.abnormal_weight = '瘦小' Or healthy_inspect.abnormal_weight = '肥胖' Or healthy_inspect.abnormal_weight = '矮小' Or healthy_inspect.abnormal_weight = '超高' Or healthy_column.abnormal2 = '异常' Or healthy_column.abnormal3 = '重度贫血') "
	if kindergarten_id != 0 {
		where += " AND healthy_inspect.kindergarten_id = ? "
		con = append(con, kindergarten_id)
	}
	...
	if date == "" {
		day_time := time.Now().Format("2006-01")
		where += " AND left(healthy_inspect.date,7) = '" + day_time + "'"
	} else {
		where += " AND left(healthy_inspect.date,7) = '" + date + "'"
	}
	...
	if err == nil {
		...
		start := (page - 1) * limit
		qb, _ := orm.NewQueryBuilder("mysql")
		sql := qb.Select("healthy_inspect.id," +
			"healthy_inspect.student_id," +
			"healthy_inspect.abnormal," +
			"healthy_inspect.abnormal_weight," +
			"healthy_inspect.abnormal_height," +
			"healthy_inspect.class_id," +
			"healthy_inspect.kindergarten_id," +
			"healthy_inspect.teacher_id," +
			"healthy_inspect.types," +
			"healthy_inspect.date," +
			"healthy_inspect.url," +
			"healthy_inspect.handel," +
			"healthy_inspect.drug_id," +
			"healthy_inspect.content," +
			"healthy_inspect.infect," +
			"healthy_inspect.created_at," +
			...
			"student.avatar," +
			"student.class_info as class_name ," +
...
func (f *Inspect) Weights(kindergarten_id int, date string) ([]orm.Params, error) {
	...
	wheres += " order by healthy_body.test_time desc "
	wheres += " limit 6 "
...
func (f *Inspect) Country(kindergarten_id int) ([]orm.Params, error) {
	...
	wheres += " order by test_time desc "
	wheres += " limit 6 "
...
func DeleteStudent(id int) map[string]interface{} {
	...
	if err = o.Read(&v); err == nil {
		var bodys Body
		o.QueryTable("healthy_body").Filter("id", v.BodyId).One(&bodys)
		if bodys.Actual > 0 {
			_, err = o.Raw("UPDATE healthy_body SET actual = actual - 1 where id = ? ", v.BodyId).Exec()
			if err == nil {
				o.QueryTable("healthy_body").Filter("id", v.BodyId).One(&bodys)
				a := float64(bodys.Actual) / float64(bodys.Total) * 100
				sql := "update healthy_body set rate = " + strconv.Itoa(int(a)) + " where id = " + strconv.Itoa(v.BodyId)
				o.Raw(sql).QueryRow(&nums)
			} else {
				err = o.Rollback()
				return nil
			}
		...
		var classs Class
		o.QueryTable("healthy_class").Filter("body_id", v.BodyId).One(&classs)
		if classs.ClassActual > 0 {
			_, err = o.Raw("UPDATE healthy_class SET class_actual = class_actual - 1 where class_id = ? and body_id = ? ", v.ClassId, v.BodyId).Exec()
			if err == nil {
				o.QueryTable("healthy_class").Filter("class_id", v.ClassId).Filter("body_id", v.BodyId).One(&classs)
				a := float64(classs.ClassActual) / float64(classs.ClassTotal) * 100
				sql := "update healthy_class set class_rate = " + strconv.Itoa(int(a)) + " where id = " + strconv.Itoa(classs.Id)
				o.Raw(sql).QueryRow(&nums)
			} else {
				err = o.Rollback()
				return nil
...
func (f *Inspect) PersonalInfo(baby_id int) (Page, error) {
	...
	if baby_id != 0 {
		...
		cnts, err := o.QueryTable("healthy_inspect").Filter("student_id", student_id).Count()
		if cnts == 0 && err == nil {
			mInfo := make([]orm.Params, 0)
	...
	if err == nil {
		...
		if _, err := o.Raw(sql, con).Values(&sxWords); err == nil {
			if len(sxWords) != 0 {
				...
				_, err := o.Raw(sql1).Values(&Heights);
...
//获取宝宝健康详情
func (f *Inspect) SituationUrl(baby_id int) (orm.Params, error) {
	o := orm.NewOrm()
	var con []interface{}
	where := "1 "
	where += "AND ( healthy_inspect.types = 1 Or healthy_inspect.types = 2 Or healthy_inspect.types = 3 ) "
	...
	//var per  orm.Params
	qb, _ := orm.NewQueryBuilder("mysql")
	sql := qb.Select("healthy_inspect.url").From(f.TableName()).
		LeftJoin("student").On("healthy_inspect.student_id = student.student_id").
		Where(where).
		OrderBy("id").Desc().
		Limit(1).Offset(0).String()
...
func (f *Inspect) AbnormalList(kindergarten_id, class_id int) ([]orm.Params, error) {
	o := orm.NewOrm()
	var body Body
	var con []interface{}
	//var cons []interface{}
	// 获取 QuerySeter 对象，user 为表名
	if err := o.QueryTable("healthy_body").Filter("kindergarten_id", kindergarten_id).OrderBy("-id").One(&body); err == nil {
		fmt.Println(body)
		where := " healthy_inspect.body_id = " + strconv.Itoa(body.Id)
		where += " AND (healthy_inspect.abnormal_weight = '肥胖' Or healthy_inspect.abnormal_weight = '瘦小') "
		if class_id != 0 {
			where += " AND healthy_inspect.class_id = " + strconv.Itoa(class_id)
		}
		//肥胖儿童
		var fBody []orm.Params
		qb, _ := orm.NewQueryBuilder("mysql")
		sql := qb.Select("healthy_inspect.abnormal_weight,student.name,student.avatar").From(f.TableName()).
			LeftJoin("student").On("healthy_inspect.student_id = student.student_id").
			Where(where).
			OrderBy("id").Desc().String()
		if _, err := o.Raw(sql, con).Values(&fBody); err == nil {
...
//宝宝健康指数
func (f *Inspect) PersonalList(babys string) ([][]orm.Params, error) {
	baby := strings.Split(babys,",")
	maps := make([][]orm.Params,0)
	for _, value := range baby {
		baby_id,_ := strconv.Atoi(value)
		o := orm.NewOrm()
		var con []interface{}
		where := "1 "
		where += "AND ( healthy_inspect.types = 1 Or healthy_inspect.types = 2 Or healthy_inspect.types = 3 ) "
