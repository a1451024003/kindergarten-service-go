...
添加园内生活
*/
func AddKindergartenLife(content string, template int, kindergarten_id int, picture string) (err error) {
	o := orm.NewOrm()
	var p []PictureNumber
	json.Unmarshal([]byte(picture), &p)
	...
	if err == nil {
		for k, _ := range p {
			l := LifePicture{LifeId: lifeId, Picture: p[k].Picture, Number: p[k].Number}
			_, err = o.Insert(&l)
		}
		if err != nil {
			return err
...
园内生活列表
*/
func ActivityPicture(kindergarten_id int) (ml interface{}, err error) {
	o := orm.NewOrm()
	var v []orm.Params
	var va []orm.Params
	qb, _ := orm.NewQueryBuilder("mysql")
	sql := qb.Select("*").From("kindergarten_life as kl").Where("kl.kindergarten_id = ?").String()
	_, err = o.Raw(sql, kindergarten_id).Values(&v)
	if err == nil {
		for _, val := range v {
			qb, _ = orm.NewQueryBuilder("mysql")
			sql = qb.Select("*").From("life_picture as lp").Where("lp.life_id = ?").String()
			_, err = o.Raw(sql, val["id"]).Values(&va)
			data := make(map[string][]interface{})
			for _, value := range va {
				if val["id"].(string) == value["life_id"].(string) {
					if strc, ok := value["number"].(string); ok {
						data[strc] = append(data[strc], value)
						val["picture"] = data
					}
				}
			}
