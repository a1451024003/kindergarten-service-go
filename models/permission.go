...
type Permission struct {
	...
	Type           int       `json:"type" orm:"column(type)" description:"类型"`
...
保存权限
*/
func AddPermission(name string, identification string, parent_id int, route string, ty int) error {
	o := orm.NewOrm()
	var v []orm.Params
	...
	if parent_id > -1 {
		...
		if leve >= 2 {
			...
			qb, _ = orm.NewQueryBuilder("mysql")
			sql = "insert into permission set parent_id = ?,name = ?,level = ?,identification = ?,created_at = ?,type = ?"
			res, err := o.Raw(sql, v[0]["id"], name, lev, identification, createTime, ty).Exec()
		...
		qb, _ = orm.NewQueryBuilder("mysql")
		sql = "insert into permission set name = ?,identification = ?,created_at = ?,type = ?"
		res, err := o.Raw(sql, name, identification, createTime, ty).Exec()
		id, _ := res.LastInsertId()
		for _, v := range r {
			qb, _ = orm.NewQueryBuilder("mysql")
			sql = "insert into permission_route set permission_id = ?,route_id = ?"
...
圈子拿的权限列表
*/
func GetGroupPer(kindergarten_id int, user_id int) interface{} {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Organizational))
	var posts []Organizational
	var Organizational []OrganizationalTree
	if _, err := qs.Filter("kindergarten_id", kindergarten_id).All(&posts); err == nil {
		for _, val := range posts {
			if val.ParentId == 0 {
				next := getGroupChilds(posts, val.Id, user_id)
				var tree OrganizationalTree
				tree.Id = val.Id
				tree.KindergartenId = val.KindergartenId
				tree.ClassType = val.ClassType
...
func getGroupChilds(posts []Organizational, id int, user_id int) (Organizational []OrganizationalTree) {
	o := orm.NewOrm()
	var v []orm.Params
	qb, _ := orm.NewQueryBuilder("mysql")
	sql := qb.Select("o.id").From("teacher as t").LeftJoin("organizational_member as om").
		On("t.teacher_id = om.member_id").LeftJoin("organizational as o").
		On("om.organizational_id = o.id").Where("isnull(t.deleted_at) and t.user_id = ?").And("om.is_principal = 0 and om.type = 0").And("o.type = 2").And("o.level = 3").And("status = 1").String()
	...
	if v == nil {
		...
		for _, val := range posts {
			ids, _ := strconv.Atoi(v[0]["id"].(string))
			if ids != val.Id {
				if val.ParentId == id {
					next := getGroupChilds(posts, val.Id, user_id)
					var tree OrganizationalTree
...
编辑权限
*/
func UpdatePermission(id int, routeId string, name string, parent_id int, identification string, ty int) (err error) {
	o := orm.NewOrm()
	o.Begin()
	var r map[string]interface{}
	...
	_, err = o.QueryTable("permission").Filter("id", id).Update(orm.Params{
		"name": name, "identification": identification, "parent_id": parent_id, "type": ty,
	})
	if err != nil {
		o.Rollback()
		return err
