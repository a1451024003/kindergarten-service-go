...
func GetUserPermissionById(user_id int, kindergarten_id int) (paginatorMap map[string]interface{}, err error) {
	...
	if err == nil {
		...
		paginatorMap["group_permission"] = GetGroupPer(kindergarten_id, user_id)
...
func GetGroupAll(user_id int, class_type int, role int, kindergarten_id int) (paginatorMap map[string]interface{}, err error) {
	...
	if role == 1 {
		...
		qb, _ := orm.NewQueryBuilder("mysql")
		sql := qb.Select("o.name as class_name", "gv.class_id", "o.class_type").
			From("group_view as gv").LeftJoin("organizational as o").
			On("o.id = gv.class_id").Where("gv.user_id = ? and o.class_type = ?").String()
